...
from src.user_behavior_risk import compute_user_behavior_risk
...
def run_alert_engine():
    ...
    # ---------------------------
    print("🔥 Starting user behavior risk scoring...")

    user_risk_df = compute_user_behavior_risk(df, alerts_df)

    user_risk_df = user_risk_df.sort_values(by="risk_score", ascending=False)

    user_risk_path = os.path.join(data_dir, "user_risk_profiles.csv")
    user_risk_df.to_csv(user_risk_path, index=False)

    print(f"User risk profiles saved to '{user_risk_path}'")
    
    return alerts_df, user_risk_df
